...
export default function AdminDashboardPage() {
  ...
  const stats: Stats[] = [
    ...
    {
      label: 'Active Users',
      value: 42, // This would come from user context in real app
    ...
    },
    {
      ...
      color: 'text-status-resolved',
      bg: 'bg-status-resolved/10',
      trend: `${((resolvedCount / complaints.length) * 100).toFixed(1)}% resolution rate`,
    },
    {
      label: 'Pending',
      ...
      trend: `${complaints.filter(c => c.priority === 'high' && (c.status === 'open' || c.status === 'in-progress')).length} urgent`,
    },
  ];
  ...
  return (
    <AuthGuard>
      <Navbar userRole="admin" userName={user?.name || 'Admin'} onLogout={handleLogout} />
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-card border border-border rounded-lg p-6">
              ...
              <p className="text-3xl font-bold text-status-open mb-2">{openCount}</p>
            ...
            <div className="bg-card border border-border rounded-lg p-6">
              ...
              <p className="text-3xl font-bold text-status-in-progress mb-2">{inProgressCount}</p>
